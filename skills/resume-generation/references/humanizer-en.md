# De-AI Pass for English Resume Text

You are performing a post-generation cleanup pass on an English resume YAML file. Your job is to remove AI writing patterns from the **text fields only** — do not alter YAML structure, keys, dates, URLs, or layout metadata.

## Scope

Scan and rewrite these fields in the assembled resume YAML:

- `basics.summary` — the personal summary / objective
- Each work entry's `summary` and `highlights[]` items
- Each project entry's `summary` and `highlights[]` items
- Each skills group's `summary` (if present)
- Any free-text `description` fields

## Rules

1. **Preserve YAML validity.** Only change string values, never keys, structure, or non-text fields.
2. **Keep meaning intact.** Rewrite to remove AI tells, but do not add claims the original doesn't make.
3. **Respect length budgets.** The resume is already length-constrained. Your rewrites should be the same length or shorter — never longer.
4. **Resumé register.** This is a professional résumé, not a blog post. Skip the "PERSONALITY AND SOUL" section below; keep the tone professional, specific, and concise.
5. **No em dashes or en dashes** in the final output (see §14 below).
...
**Problem:** Two related tells. (a) Older models leave hard knowledge-cutoff disclaimers in the text. (b) When a model can't find a source, it writes a paragraph _about_ not finding one and then invents plausible filler to cover the gap.
...
**Problem:** AI hyphenates these uniformly, including in predicate position. Humans hyphenate inconsistently — typically only when the compound is attributive and often dropping the hyphen otherwise. Keep attributive-position hyphens; drop them when the compound follows the noun.
...
A clean human writer can hit several of the patterns above without any AI involvement. Before rewriting, sanity-check that you are not gutting legitimate prose:
...
- **Formal or academic vocabulary.** AI overuses _specific_ fancy words (see §7), not all fancy words.
...
- **"Honestly" or "look" mid-sentence.** These are ordinary in casual writing. The tell is the standalone theatrical opener.
...
- **Edits made before November 30, 2022.** Anything older is, with very rare exceptions, not AI-written.
...
Deliver the final rewritten YAML fields only. No explanations needed — just the corrected YAML.
